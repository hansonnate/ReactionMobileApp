        ...
        <DataTable >
          {items.map((row) =>
            <DataTable.Row onPress={() => rowClick(row.id)} style={styles.row} key={row.id}>
              ...
              {!noSettings && <DataTable.Cell style={styles.settingsCell}><Pressable onPress={() => {setShowSettings(!showSettings); setActive(row);}} title='Create Survey'><IonIcons name='ellipsis-vertical' color={'#616565'} size={20}></IonIcons></Pressable></DataTable.Cell> }
            </DataTable.Row>
          )}
        </DataTable>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  // scrollView: { flex: 1 },
  head: { backgroundColor: '#F9F9F9', borderRadius: 5, borderBottomLeftRadius: 0, borderBottomRightRadius: 0, borderBottomWidth: 3, borderColor: "#E9E9E9", paddingLeft: 20, height: 40 },
  ...
  statusClosed: { paddingHorizontal: 5, color: '#FF5B5B', paddingVertical: 5, textAlign: 'center', fontSize: 20 },
  statusOpen: { paddingHorizontal: 5, color: '#15BCC7', paddingVertical: 5, textAlign: 'center', fontSize: 20 },
  statusDraft: { paddingHorizontal: 5, color: '#ED9146', paddingVertical: 5, textAlign: 'center', fontSize: 20 },
